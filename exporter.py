#!/usr/bin/env python3
"""
Общая логика экспорта Telegram: БД, клиент, обработка медиа и загрузка чатов.

Модуль безопасен для импорта: не создаёт папки, не настраивает логи и не завершает процесс.
"""
import os
import sys
import json
...
from telethon.tl.types import (
    ...
    MessageMediaDice, PeerChannel, PeerChat, PeerUser, CodeSettings
)
...
DOWNLOADABLE_MEDIA = (
    MessageMediaPhoto, MessageMediaDocument, MessageMediaGame,
    MessageMediaInvoice, MessageMediaGeo, MessageMediaContact, MessageMediaDice,
)
...
class Settings:
    ...
    def max_file_size_bytes(self) -> Optional[int]:
        return mb_to_bytes(self.max_file_size_mb)


def mb_to_bytes(mb: Optional[float]) -> Optional[int]:
    if mb is None:
        return None
...
def setup_logging(name: str = 'TelegramExporter') -> logging.Logger:
    LOG_DIR.mkdir(exist_ok=True)
    log_filename = LOG_DIR / f'{name.lower()}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    logging.basicConfig(
        level=logging.INFO,
...
def load_settings() -> Settings:
    ...
    chats = getattr(app_config, 'CHATS', None)
    if chats is None:
        print('Ошибка: файл config.py не найден или не содержит CHATS')
        sys.exit(1)

    timezone = getattr(app_config, 'SCHEDULE_TIMEZONE', None)
    if isinstance(timezone, str):
        timezone = timezone.strip() or None

    cron = getattr(app_config, 'SCHEDULE_CRON', None)
    if isinstance(cron, str):
        cron = cron.strip() or None

    return Settings(
        chats=chats,
        download_attachments=bool(getattr(app_config, 'DOWNLOAD_ATTACHMENTS', True)),
        max_file_size_mb=getattr(app_config, 'MAX_FILE_SIZE_MB', None),
        schedule_cron=cron,
        schedule_timezone=timezone,
        schedule_run_on_start=bool(getattr(app_config, 'SCHEDULE_RUN_ON_START', True)),
    )
...
class Database:
    ...
    async def list_attachments_with_chats(self):
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                '''
                SELECT a.id, a.file_path, a.file_name, a.file_size, a.mime_type, a.telegram_file_id,
                       m.message_id AS tg_message_id, c.telegram_id AS chat_telegram_id,
                       c.username, c.title
                FROM attachments a
    ...
    async def update_attachment_size(self, attachment_id: int, file_size: int):
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                'UPDATE attachments SET file_size = ? WHERE id = ?',
...
def append_file_links(text: Optional[str], attachments: Optional[List[Dict]]) -> Optional[str]:
    """Добавляет в текст относительные ссылки только на реально скачанные файлы."""
    if not attachments:
        return text

    links = []
    for att in attachments:
        if not att.get('downloaded'):
            continue
...
def get_media_size(media) -> Optional[int]:
    """Размер вложения в байтах, если Telegram его сообщает до скачивания."""
    ...
    if photo:
        ...
        for item in getattr(photo, 'sizes', None) or []:
            ...
            if size:
                known.append(int(size))
            progressive = getattr(item, 'sizes', None)
            if progressive:
                known.append(int(max(progressive)))
        return max(known) if known else None
    return None


def _media_mime_and_id(media) -> tuple[Optional[str], Optional[str]]:
    mime_type = None
    telegram_file_id = None
    if hasattr(media, 'document') and media.document:
        mime_type = getattr(media.document, 'mime_type', None)
        telegram_file_id = getattr(media.document, 'id', None)
    elif hasattr(media, 'photo') and media.photo:
        mime_type = 'image/jpeg'
        telegram_file_id = getattr(media.photo, 'id', None)
    return mime_type, str(telegram_file_id) if telegram_file_id else None


def _media_file_name(media, message_id: int) -> str:
    if hasattr(media, 'document') and media.document:
        ...
        return f'file_{message_id}.{ext or "bin"}'
...
def should_skip_by_size(file_size: Optional[int], max_file_size_bytes: Optional[int]) -> bool:
    if max_file_size_bytes is None or file_size is None:
        return False
    return file_size > max_file_size_bytes


def db_attachment(meta: Dict, downloaded: bool) -> Dict:
    return {
...
async def process_media(
    ...
    """Метаданные вложения всегда; файл на диск — только если разрешено политикой."""
    if not message.media:
        return None

    media = message.media
    if not isinstance(media, DOWNLOADABLE_MEDIA):
        return None

    file_name = _media_file_name(media, message_id)
    mime_type, telegram_file_id = _media_mime_and_id(media)
    file_size = get_media_size(media)
    abs_path = FILES_DIR / str(chat_telegram_id) / str(message_id) / file_name
    relative_path = abs_path.relative_to(BASE_DIR).as_posix()
    meta = {
    ...
    if abs_path.exists():
        meta['file_size'] = abs_path.stat().st_size
        logger.info(f'Файл уже существует: {abs_path}')
        return [db_attachment(meta, downloaded=True)]

    if not download_attachments:
        logger.info(f'Пропуск скачивания вложения (режим без файлов): {relative_path}')
        return [db_attachment(meta, downloaded=False)]

    if should_skip_by_size(file_size, max_file_size_bytes):
        logger.info(
            f'Пропуск файла {file_name}: {human_size(file_size)} > лимит '
    ...
    try:
        downloaded_path = await message.download_media(file=str(abs_path))
        if downloaded_path:
            size = Path(downloaded_path).stat().st_size
            meta['file_size'] = size
            relative = Path(downloaded_path).relative_to(BASE_DIR).as_posix()
            meta['file_path'] = relative
            logger.info(f'Скачан файл: {relative} ({size} байт)')
            return [db_attachment(meta, downloaded=True)]
...
async def load_chat(
    ...
    while True:
        ...
        for msg in messages:
            ...
            if msg.media:
                attachments_info = await process_media(
                    msg,
                    chat_telegram_id,
                    msg.id,
...
async def request_login_code(client: TelegramClient, phone: str, api_id: int, api_hash: str):
...
async def authorize_client(
    ...
    При interactive=False без готовой сессии вход не запрашивается.
    """
    ...
    if not interactive:
        raise RuntimeError(
            'Нет активной сессии Telegram. Сначала выполните python main.py для входа.'
        )
    ...
    sent = await request_login_code(client, phone, api_id, api_hash)
...
async def run_export(
    ...
    try:
        await authorize_client(client, api_id, api_hash, interactive=interactive)
    except Exception as e:
